...
export type UserBookItemUrlBuilderType = {
    getUserBooks: (pageNumber: number, pageSize: number, userId: number, includeBookAuthors?: boolean) => string
...
const UserBookItemUrlBuilder: UserBookItemUrlBuilderType = {
    searchItems: (query: string) => 
        `${appConfig.apiPaths.userBookItem}/user-books/2?pageNumber=${1}&pageSize=${10}&includeBookAuthors=${true}`,

    getUserBooks: (pageNumber: number, pageSize: number, userId: number, includeBookAuthors: boolean = true) => 
    ...
    uploadBookImage: () => 
        `${appConfig.apiPaths.userBookItem}/Image`,
