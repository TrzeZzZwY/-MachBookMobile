...
export default function SearchablePicker<T extends SearchableType>({
  onChange,
  ...
  mapper,
  searchPlaceholder,
  sideButtonAction = null,
}: SearchablePickerProps<T>) {
  const axios = useAxios();
  const [searchTimeout, setSearchTimeout] = useState<null | ReturnType<
    typeof setTimeout
  >>(null);
  ...
  const seatchItems = (query: string) => {
    axios
      .get<Pagination<T>>(service.searchItems(query))
  ...
  return (
    <View>
      <View className="mt-2 flex-row items-center justify-between">
        ...
        {sideButtonAction !== null ? (
          <View className="w-[18%] ml-[2%]" style={{ height: sectionHeight }}>
            <TouchableOpacity
              className="w-full h-full flex items-center justify-center bg-neutral-900 rounded-md"
