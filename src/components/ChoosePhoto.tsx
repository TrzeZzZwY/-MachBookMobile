...
export default function ChoosePhoto({ onSuccessfullUpload }: ChoosePhotoProps) {
  ...
  const getMbValueFromFile = (asset: ImagePickerAsset | null): string => {
    if (asset === null || asset?.fileSize === undefined) return "";
  ...
  const handleImageUpload = async (
    ...
    if (!image.mimeType || !image.fileName) return null;
  ...
  return (
    <View>
      ...
      <View>
        <TouchableOpacity
          className="h-36 bg-neutral-100 rounded-lg mt-2 flex items-center justify-center overflow-hidden"
          onPress={handlePhotoPick}
        >
          {asset === null ? (
            <Upload fill={"#D4D4D4"} className="h-10 w-10" />
          ) : (
            <View className="w-full h-full relative">
              <Image className="h-full w-full" src={asset.uri} />
              <View className="absolute top-2 right-2 z-2">
                <Text className="bg-neutral-100 rounded-sm px-2 py-1 font-roboto-bold">
        ...
        {asset !== null ? (
          <View className="flex flex-row justify-between mt-2">
            <Text className="font-roboto-light px-2 py-1">
              {asset?.fileName}
            </Text>
            <Text className="font-roboto-bold bg-neutral-100 px-2 py-1 rounded-md">
              {getMbValueFromFile(asset)}
            </Text>
          </View>
        ) : null}
      </View>
